...
def is_duplicated_number(three_digit):
    ...
    digitNum = int(three_digit)
    one = digitNum//100
    two = digitNum%100//10
    three = ((digitNum%100)%10)
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    digitUserNum = int(user_input_number)
    digitRandNum = int(random_number)
    strike = 0
    ball = 0
    
    userOne = digitUserNum // 100
    userTwo = digitUserNum % 100 // 10
    userThree = ((digitUserNum % 100) % 10)
    randOne = digitRandNum // 100
    randTwo = digitRandNum % 100 // 10
    randThree = ((digitRandNum % 100) % 10)
    
    if userOne == randOne:
        strike += 1
    if userTwo == randTwo:
        strike += 1
    if userThree == randThree:
        strike += 1
    if userOne == randTwo or userOne == randThree:
        ball += 1
    if userTwo == randOne or userTwo == randThree:
        ball += 1
    if userThree == randOne or userThree == randTwo:
        ball += 1
    result = [strike, ball]
    # ==================================
    return result
...
def main():
    ...
    while True:
        ...
        if user_input == "0":
            ...
            playingResult = get_strikes_or_ball(user_input, random_number)
            print("Strikes : ", playingResult[0], " Balls : ", playingResult[1])
